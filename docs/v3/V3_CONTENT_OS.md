...
**Status:** proposal. **Revised in v3.1** (§6 phase 3 targets the Canva production
system) and **in v3.2** (new §7 — Remotion, and the create-once distribution
loop). Internal tooling — never a visitor-facing feature.
...
## 7 · Remotion and the distribution loop *(new in v3.2)*

**Not a launch blocker. Documented so the boundary is set before anyone crosses it.**
...
one real thing Adel did
        ...
        ├─→ share card           generated from tokens             (today, achievements)
        └─→ newsletter item      existing newsletter               (today)
```

**Create once → distribute intelligently everywhere.**

### Where Remotion sits

| dimension | position |
|---|---|
| where it runs | Adel's machine or CI, at authoring time |
| what it produces | MP4, WebM, PNG — **files** |
| what the website sees | a finished file, like any other asset |
| runtime coupling | **none.** Not in `site/package.json`. Its own workspace |

Good fits: branded Reel and Short templates · animated explainers · dynamic
motion graphics · share assets · data-driven video, such as an achievement card
rendered as a five-second animation.

### The one obligation v3.2 adds to the payload schema
...
distribution — and the point of the loop is that attention lands somewhere Adel
owns. The existing brand-rule gates are unchanged, including the one that matters
most: **the pipeline may never write Adel's take.**
