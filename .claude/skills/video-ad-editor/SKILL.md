...
description: محرّر فيديو production-ready متعدد الأنماط لـ AdelTechTalks: Talking Head/Reels، ASMR & Product Unboxing مع الحفاظ على أصوات المايك الطبيعية، Product Demo، Product Comparison، وMotion Carousel من 5–6 شرائح فيديو قصيرة (~5 ثوانٍ لكل Slide). يشيل السكتات، يفرّغ الكلام، يركّب captions/semantic motion عند الحاجة، أو يعمل product-first montage بلا captions/music حسب الـformat. ALWAYS use this skill when the user asks to edit video, reel, talking head, ASMR/unboxing, product demo/comparison, or motion carousel video slides. Static-only carousel design remains in the brand/carousel design system.
---

# مونتاج إعلان الفيديو — بلا برنامج مونتاج

## الفكرة
المونتاج كله كود، لكن **v3 صار format-aware**: ما يفرض نفس المعالجة على كل فيديو. فيديو الكلام يستخدم القص + التفريغ + الكابشن + المشاهد. فيديو المنتج/ASMR يستخدم تحليل اللقطات + ترتيب القصة + clean cuts + الصوت الطبيعي + brand assets المعتمدة. المخرَج دائماً MP4 جاهز للنشر.

## v3.1 — اختار الـformat بنفسك، ولا تسأل المستخدم عن التقنية

ابدأ من نية المستخدم والمادة الخام. اقرأ ملف الـformat المناسب من `formats/` قبل الشغل:

- **Talking Head / Reel** → `formats/talking-head.json` + الـpipeline الأصلي بالخطوات 0–11 أدناه.
- **ASMR Unboxing / Product Unboxing** → `formats/asmr-unboxing.json` + مسار ASMR أدناه.
- **Motion Carousel** → `formats/motion-carousel.json` + مسار Motion Carousel أدناه. هذا ليس static carousel: كل Slide فيديو قصير مستقل، افتراضياً 5 ثوانٍ.
- **Product Comparison** → `formats/comparison.json`. لو فيه كلام، استخدمه كـspine ثم ابنِ split-screen/matched shots/spec cards كـsemantic scenes. لو مفيش كلام، ابنِ storyboard من معايير المقارنة واللقطات.
- **Product Demo / Feature Spotlight** → `formats/product-demo.json` مع product-first pacing وproof shots حقيقية.
- لو المادة mixed (كلام + unboxing)، خلّي الكلام هو spine للفيديو وأدخل لقطات المنتج كـB-roll، إلا إذا طلب صراحة ASMR.

**قاعدة عامة:** نفس الـbrand system يخدم كل formats، لكن قواعد الصوت والكابشن والـmotion تختلف حسب الـformat. ممنوع إجبار ASMR على captions، وممنوع إجبار Talking Head على الصمت، وممنوع تحويل Motion Carousel إلى slideshow static.

### AdelTechTalks Brand Contract
اقرأ `BRAND_SYSTEM.md` و`brand/adeltechtalks.bootstrap.json` قبل أي output. Figma هو الـvisual source of truth. لا تخترع لون/خط/signature جديد إذا فيه approved asset. الملفات المعتمدة تتحط في `assets/`، والـskill يطبقها.

### مسار ASMR Unboxing — الافتراضي للمنتجات

**القاعدة:** لا موسيقى، لا captions، لا transitions استعراضية. **المايك مش muted**: صوت المنتج والتغليف والخرفشة والـpeel والـclick هو البطل. حافظ على الصوت المسجّل كما هو افتراضياً؛ لا noise gate ولا denoise ولا compression ولا loudness normalization إلا لو المستخدم طلب. النص إن ظهر يكون minimal جداً ومن الـbrand system فقط.

1. جهّز مجلد الشغل، وحط كل اللقطات الأصلية داخل `<work>/raw/`. لا تطلب من المستخدم يرتبها أو يسميها.
2. افحص الأدوات بوضع ASMR:
```bash
bash scripts/00_setup.sh --mode asmr
```
3. ابنِ inventory + representative frames:
```bash
python3 scripts/12_asmr_inventory.py <work>
```
4. اقرأ `asmr-inventory.json` وشاهد representative frames. صنّف كل لقطة إلى: `hook_or_hero`, `sealed_package`, `opening_action`, `first_reveal`, `accessories`, `macro_details`, `interaction_or_power_on`, `hero_close`.
5. اختَر أفضل take فقط من كل لحظة. لا تستخدم لقطة لمجرد أنها موجودة. اعمل pacing سريع لكن مريح، عادة 20–35 ثانية للـshort-form ما لم يطلب غير ذلك.
6. اكتب `<work>/asmr-plan.json` بالاعتماد على `templates/asmr-plan.sample.json`. كل segment لازم يحدد الملف، start/end، السرعة، volume، وlabel.
7. قواعد القطع:
   - clean cut أو match cut فقط افتراضياً.
   - speed ramps subtle فقط؛ لا تستخدمها على كل لقطة.
   - لا تعمل zoom رقمي لمجرد خلق حركة.
   - حافظ على ترتيب منطقي: hook → package → opening → reveal → details → interaction → hero close.
   - لو لقطة الصوت فيها click/peel/snap مهم، لا تقطع قبل peak الصوت مباشرة.
8. الـbrand assets اختيارية لكن لو موجودة تُطبق تلقائياً: `assets/opener.mp4`, `assets/signature.png`, `assets/endcard.mp4`. لا تخترع بدائل إذا ناقصة.
9. جمّع الفيديو:
```bash
python3 scripts/13_asmr_assemble.py <work> <work>/asmr-final.mp4
```
10. راجع النتيجة بصرياً وصوتياً. اختبر أن الصوت الطبيعي واضح، ما فيه click cuts، والـsignature بعيد عن UI safe zones. لو فيه مشكلة عدّل `asmr-plan.json` وأعد التجميع فقط — لا تعيد التحليل كله.
11. سلّم الفيديو النهائي + ملخص قصير لما اتعمل.

**ممنوع في ASMR افتراضياً:** Whisper، captions، background music، SFX صناعية، talking-head zoom pattern، أو motion graphics الثقيلة.

## مسار Motion Carousel — 5–6 فيديوهات قصيرة بدل صور ثابتة

**الهدف:** كل Slide يبقى فيديو مستقل قصير (افتراضياً 5 ثوانٍ) يمكن رفعه كجزء من Carousel، مع نسخة stitched preview للمراجعة. الفكرة ليست transition بين Slides؛ كل Slide لازم يقدر يعيش لوحده ويشرح نقطة واحدة.

1. جهّز `<work>/media/` وحط لكل Slide صورة أو فيديو قصير. لو المستخدم أعطاك لقطات أطول، اختَر أفضل ~5 ثوانٍ لكل فكرة قبل التصميم.
2. اكتب `<work>/carousel-plan.json` من `templates/motion-carousel-plan.sample.json`.
3. لكل Slide حدّد: `headline`, `body` الاختياري، `media`, `media_type`, `motion`, `rtl`, `duration`.
4. الـmotion لازم يخدم الفكرة: `hero_push` للـhero/product reveal، `macro_drift` للتفاصيل، `parallax_*` للمشهد العميق، `feature_reveal` لإظهار feature، و`clean_hold` لما الصورة نفسها قوية.
5. العربي RTL-first. لا تحشر أكثر من فكرة رئيسية واحدة في Slide، ولا تحول النص إلى paragraph طويل.
6. الصوت افتراضياً muted لأن Carousel يُستهلك slide-by-slide. لو الصوت الطبيعي نفسه هو الفكرة، يجوز تشغيله بقرار واعٍ؛ ولو فيه Voiceover، وحّد مستواه عبر كل Slides.
7. جهّز renderer:
```bash
bash scripts/15_motion_carousel.sh <work> setup
```
8. لو احتجت مراجعة حية:
```bash
bash scripts/15_motion_carousel.sh <work> studio
```
9. الرندر النهائي:
```bash
bash scripts/15_motion_carousel.sh <work> render
```
ينتج:
- `<work>/exports/slides/01.mp4`, `02.mp4`... كل Slide منفصل.
- `<work>/exports/carousel-preview.mp4` نسخة مجمّعة للمراجعة فقط.

**قواعد AdelTechTalks للـMotion Carousel:** product/video أولاً، text ثانياً، signature صغيرة، no gimmicky transitions، وكل Slide لازم يكون visually satisfying حتى لو المستخدم وقف عليه لوحده.

## مسار Product Comparison

- حدّد أولاً معايير المقارنة الحقيقية (مثلاً image quality, stabilization, audio, AI feature, workflow, price/value) ولا تخترع score بلا proof.
- طابق اللقطات قدر الإمكان: نفس الزاوية/الإضاءة/الحركة أو اذكر بوضوح إن ظروف الاختبار مختلفة.
- لو فيه Talking Head/VO: فرّغ الكلام، شيل التكرار، وخلّي المقارنة البصرية مرتبطة بالجملة المنطوقة.
- استخدم split-screen فقط لما المقارنة المتزامنة فعلاً مفيدة؛ غير كده A → B أو matched cuts أو spec card أو winner-by-use-case أوضح.
- حافظ على labels واضحة للمنتجين طول أي split-screen.
- الناتج النهائي لازم يشرح **مين أفضل لمين وليه**، مش بس مين كسب رقمياً.

## مسار Product Demo / Feature Spotlight

- افتح بالنتيجة أو الـfeature في الاستخدام الحقيقي، ثم فسّرها.
- استخدم B-roll الحقيقي للمنتج، close-ups، screen recordings أو proof shots حسب الحاجة.
- النص والموشن يوضحان، لا يغطّيان المنتج.
- لو الفيديو بلا كلام، استخدم minimal text. لو فيه كلام، فعّل captions حسب الـTalking Head rules.
...
| `11_behind_text.js` + `personmask.swift` | الكلام يمرّ ورا الشخص بالكشيدة | الخفيف |
